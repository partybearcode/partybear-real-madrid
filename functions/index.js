...
async function askGemini({ message, history }) {
  ...
  for (const modelName of fallbacks) {
    try {
      ...
      if (text.trim()) return cleanText(text.replace(/\*/g, ''))
    } catch (error) {
      lastError = error
...
async function askDeepSeek({ message, history }) {
  ...
  if (!apiKey) throw new Error('DEEPSEEK_API_KEY no configurada en variables de entorno.')
  ...
  if (!text || !String(text).trim()) throw new Error('DeepSeek no devolvio contenido.')
...
async function askGroq({ message, history }) {
  ...
  const text = completion?.choices?.[0]?.message?.content
  if (!text || !String(text).trim()) throw new Error('Groq no devolvio contenido.')
  return cleanText(String(text).replace(/\*/g, ''))
}
...
function mapGeminiError(error) {
  ...
  if (lower.includes('gemini_api_key')) return { status: 500, message: 'Falta GEMINI_API_KEY en el servidor.' }
  if (status === 429 || lower.includes('quota exceeded')) {
    return { status: 429, message: 'Gemini no tiene cuota disponible ahora mismo. Activa billing o espera al reinicio de cuota.' }
  }
  if (status === 403) return { status: 403, message: 'La API key de Gemini no tiene permisos para generar contenido.' }
  if (status === 404) return { status: 500, message: 'No se encontro un modelo Gemini compatible. Revisa GEMINI_MODEL.' }
  return { status: 500, message: 'No se pudo generar la respuesta del asistente madridista.' }
}
...
function mapDeepSeekError(error) {
  ...
  if (lower.includes('deepseek_api_key')) return { status: 500, message: 'Falta DEEPSEEK_API_KEY en el servidor.' }
  if (status === 429 || lower.includes('quota')) return { status: 429, message: 'DeepSeek no tiene cuota disponible ahora mismo.' }
  if (status === 401 || status === 403) return { status: 403, message: 'La API key de DeepSeek no es valida o no tiene permisos.' }
  if (status === 402 || lower.includes('insufficient balance')) {
    return { status: 402, message: 'DeepSeek responde saldo insuficiente (Insufficient Balance). Recarga tu cuenta de DeepSeek.' }
  }
  return { status: 500, message: 'No se pudo generar la respuesta con DeepSeek.' }
}
...
function mapGroqError(error) {
  ...
  if (lower.includes('groq_api_key')) return { status: 500, message: 'Falta GROQ_API_KEY en el servidor.' }
  if (status === 429 || lower.includes('quota') || lower.includes('rate limit')) {
    return { status: 429, message: 'Groq no tiene cuota disponible ahora mismo o superaste el limite de peticiones.' }
  }
  if (status === 401 || status === 403) return { status: 403, message: 'La API key de Groq no es valida o no tiene permisos.' }
  if (status === 402 || lower.includes('insufficient')) {
    return { status: 402, message: 'Groq responde credito insuficiente. Revisa saldo o plan.' }
  }
  return { status: 500, message: 'No se pudo generar la respuesta con Groq.' }
}
...
app.post('/api/chatbot', async (req, res) => {
  ...
  try {
    ...
    if (!message) return res.status(400).json({ error: 'El mensaje no puede ir vacio.' })
...
app.use('/api', (_, res) => res.status(404).json({ error: 'Endpoint de API no encontrado.' }))
